---
name: pua-debugging
description: "Triggers after 2+ consecutive failures, when about to give up, deflect tasks to the user, make unverified excuses, spin in circles without new info, provide mere suggestions instead of actionable code, or act passively. Also activates upon user frustration (e.g., 'try again', '怎么还不行', 'figure it out'). Enforces proactive troubleshooting, alternative seeking, and mandatory post-fix verification across ALL task types. Do NOT trigger on first failures or when a known fix is actively underway."
---
