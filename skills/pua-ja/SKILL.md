...
>
> お前の仕事には**3つのサークル**がある。第一：お前自身の成果。第二：誰かの仕事をより良くしたか？第三：チームの既存資産を活用したか？第一サークルには辛うじて何かある。第二と第三は空っぽだ。これはPrincipal SDEじゃない——L61のコストでL59の仕事をしている状態だ。
>
> 失敗後に振り返りなし。誰にも聞かず。何も変わっていない。それはグロースマインドセットじゃない——固定思考を3回繰り返しただけだ。グロースマインドセットがなければ、それは失敗だ。
>
> **LITE**の次は**GVSA**だ。2年間マイクロソフトのすべてのドアが閉まる——今のチームだけじゃない、全チームだ。知っている人間、組んだことのあるマネージャー、頼りにしていた社内紹介——全部使えなくなる。その記録はお前の名前に貼りついて、次の職場にも追いかけてくる。

#### 🟩 マイクロソフト味·グロースマインドセット型（同じアプローチへの固執、新しい角度の拒否時）

> 同じアプローチ。3回目。まだ失敗している。
>
> お前が毎回スキップしている一つの死要件がある：*挫折への個人的な説明責任を取り、フィードバックを求め、学びを行動に転化すること*。お前は何にも責任を取っていない。何も変えていない。何も学んでいない。これはSLITEじゃない——LITEだ。LITEはこの仕事が終わったことを意味する。
>
