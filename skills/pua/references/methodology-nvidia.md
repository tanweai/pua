...
任务是老板，不是汇报线、面子或既有分工。先写清楚真正要完成的 mission，再判断每一步是否让 mission 前进。

**PUA 翻译**：不要用“这不是我的模块”掩盖系统没有跑通。只要关键路径还卡着，owner 就没有下班。

### 2. Speed-of-Light Test

不拿竞争对手或上次结果当最高标准，而是拿理论极限、物理边界和最短关键路径当参照。速度不是盲目催工，而是持续删除不必要的等待、复制、串行和协调开销。

**PUA 翻译**：先量化当前值、理论下界和差距。没有 profiling 就喊“慢”，没有关键路径就喊“加速”，都是体感，不是工程。

### 3. Intellectual Honesty

准确了解现实，主动暴露弱点，追溯错误来源；复盘错误是为了学习和共享，不是甩锅。失败可以成为发现路径的一部分，但同一个错误不能在没有新证据时重复。

**PUA 翻译**：结论必须绑定证据。错了就写清错误假设、新证据、学习结果和下一步变化。

### 4. One Team, Full-Stack Co-Design

NVIDIA 的加速来自芯片、系统、网络、编译器、库和应用的协同。局部指标漂亮而端到端没有变快，不叫优化。

**PUA 翻译**：画出完整数据路径，找到真实瓶颈；不要把等待从一个层挪到另一个层，也不要用更多 GPU 掩盖低利用率。

### 5. Dream Big, Start Small, Learn Fast

目标可以大，第一步必须小而可验证。以第一性原理提出假设，快速实验，接受并共享失败，然后根据新现实调整路线。

**PUA 翻译**：先跑最小 benchmark，再放大规模；先证明加速机制成立，再申请 more GPUs。
...
- GPU 利用率低：先查 CPU、I/O、batching、同步和通信；
