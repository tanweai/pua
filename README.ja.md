...
| コアskillの直接呼び出し | 対話で `$pua:pua-ja` と入力 | `skills/pua-ja/SKILL.md` |
| 名前空間コマンド | 対話で `$pua:p7`、`$pua:pro`、`$pua:flavor` などを入力 | `skills/` のcanonical skillと `skills/flavor/` のような同サフィックスbridge |
...
独立した `pua-enforcer` agent spec はこのリポジトリではまだ公開していません。現時点では方法1を使ってください。
...
> v3フック機能はClaude Code専用です。Codex ではフックは使いませんが、同じ名前空間コマンド面を `$pua:xxx` で使えます。
...
| **Codex CLI** | あり（skillのdescription照合） | `$pua:pua-ja`、`$pua:p7`、`$pua:pro`、`$pua:flavor` など |
...
> **注意：** Claude Code は `/pua:p7` のようなスラッシュコマンドを使い、Codex は同じ名前空間コマンドを `$pua:p7`、`$pua:pro`、`$pua:flavor` のように使います。
...
> Claude Code は `/pua:on` のようなスラッシュコマンドを使い、Codex は対応する名前空間コマンド `$pua:on` を使います。
>
> 各サブコマンドには Claude Code 上で2つの等価な呼び出し方法があります：スタンドアロン（`/pua:on`）またはメインコマンド経由（`/pua:pua on`）。Codex ではスタンドアロンのコマンド面を `$pua:xxx` として公開します。
