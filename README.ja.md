...
```json
{
  ...
  "plugins": {
    "pua@pua-skills": [
      {
        "scope": "user",
        "installPath": "/ローカルの絶対パス/.claude/plugins/pua",
        "version": "2.9.0"
      }
    ]
  }
...
`installPath` にはローカル checkout の絶対パスを指定してください。Claude Codeを再起動して反映。更新は `~/.claude/plugins/pua` で `git pull` を実行。
...
| 直接呼び出し | 対話で `$pua` と入力 | インストール済みskill |
| 直接サブコマンド | 対話で `$pua:p7`、`$pua:on`、`$pua:survey` などを入力 | インストール済みskills |
| 手動プロンプト | 対話で `/prompts:pua` と入力 | インストール済みprompt |

Codex のインストールはリポジトリ内の `codex/` エクスポート層だけを使うため、共有の Claude Code skill 面は変更しません。インストーラスクリプトがエクスポート項目の生成・検証・リンクを一元化するので、新しい Codex サブコマンドが増えても、ユーザーが手でリンク一覧を書き足す必要はありません。
...
| **Codex CLI** | あり（skillのdescription照合） | `$pua`、`$pua:xxx`、または `/prompts:pua` |
...
> **注意：** Codex CLI でも `$pua:p7` や `$pua:on` のような namespaced サブコマンドが使えます。下の Hooks は引き続き Claude Code 専用です。
...
/pua:mama       → 中国式ママ小言モード（ルール不変、口調だけ変更）
/pua:shot       → v2 濃縮単一ファイル（449行、依存ゼロ、全量注入）
...
### コマンド（Claude Code / Codex CLI）

> Claude Code は `/pua:...`、Codex CLI は同じ名前空間を `$pua:...` で呼び出します。
>
> メインコマンド経由の形も等価です。Claude Code では `/pua:pua on`、Codex では `$pua` から同じルーティングに入れます。
...
| `/pua:mama` | 中国式ママ小言モード — コアルールはそのまま、口調だけ変更 |
| `/pua:shot` | v2 濃縮単一ファイル — 449行、依存ゼロ、sub-agent注入向け |
