...
```json
{
  ...
  "plugins": {
    "pua@pua-skills": [
      {
        "scope": "user",
        "installPath": "/你的本地绝对路径/.claude/plugins/pua",
        "version": "2.9.0"
      }
    ]
  }
...
`installPath` 必须填写你本地 checkout 的绝对路径。重启 Claude Code 即可生效。更新时在 `~/.claude/plugins/pua` 目录执行 `git pull`。
...
| 直接调用 | 对话中输入 `$pua` | 已安装 skill |
| 直接子命令 | 对话中输入 `$pua:p7`、`$pua:on`、`$pua:survey` 等 | 已安装 skills |
| 手动 prompt | 对话中输入 `/prompts:pua` | 已安装 prompt |

Codex 安装只从仓库里的 `codex/` 导出层读取，因此不会改动共享的 Claude Code skill 面。安装脚本会统一生成、校验并链接导出项，所以后续新增 Codex 子命令时，用户不需要手动维护一串链接命令。
...
| **Codex CLI** | 是（skill description 匹配） | `$pua`、`$pua:xxx` 或 `/prompts:pua` |
...
> **注意：** Codex CLI 现在也支持 `$pua:p7`、`$pua:on` 这类 namespaced 子命令。下面的 Hooks 仍然是 Claude Code 专属。
...
### 命令（Claude Code / Codex CLI）

> Claude Code 使用 `/pua:...`，Codex CLI 使用同一套 namespaced 入口 `$pua:...`。
>
> 主命令透传形式依然等价：Claude Code 中是 `/pua:pua on`，Codex 中可通过 `$pua` 进入主路由。
