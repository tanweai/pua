...
| 直接调用核心 skill | 对话中输入 `$pua:pua` | `skills/pua/SKILL.md` |
| 命令风格调用 | 对话中输入 `$pua:p7`、`$pua:pro`、`$pua:flavor` 等 | `skills/` 中的 canonical skill，加上 `skills/flavor/` 这类同后缀 bridge |
...
仓库当前还没有发布独立的 `pua-enforcer` agent 文件，现阶段请先使用方式一。
...
| **Codex CLI** | 是（skill description 匹配） | `$pua:pua`、`$pua:p7`、`$pua:pro`、`$pua:flavor` 等 |
...
> **注意：** Claude Code 使用 `/pua:p7` 这类斜杠命令；Codex 直接暴露同名命名空间命令，例如 `$pua:p7`、`$pua:pro`、`$pua:flavor`。
...
> Claude Code 使用 `/pua:on` 这类斜杠命令；Codex 使用同名命名空间命令 `$pua:on`。
>
> 每个子命令在 Claude Code 中有两种等价调用方式：独立命令（`/pua:on`）或通过主命令传参（`/pua:pua on`）。Codex 直接暴露独立命令面，即 `$pua:xxx`。
