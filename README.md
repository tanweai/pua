...
除了 skill 的语义匹配触发外，插件还包含一套 **hook 旁路监测** 机制，基于 AI 的实际行为模式自动检测打转和放弃：

- **PostToolUse hook** — 每次工具调用后记录调用指纹，检测重复失败模式（相同命令/相同错误出现 ≥ 2 次）
- **Stop hook** — 当 Claude 试图退出时，检查是否存在打转模式或放弃关键词，自动拦截并注入对应等级的 PUA 干预

**与 skill 的互补关系：**

| | Skill（预防层） | Hook（拦截层） |
|---|---|---|
| 触发方式 | 语义匹配 description | 行为模式检测 |
| 作用时机 | AI 生成回复前 | AI 试图退出时 |
| Context 消耗 | ~5KB prompt | 几十字 reason |
| Token 消耗 | 注入完整方法论 | 零（纯 shell 脚本） |

两者互补：skill 负责"预防"（让 AI 知道不该放弃），hook 负责"兜底"（在 AI 已经在放弃时拦截）。
