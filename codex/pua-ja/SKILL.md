...
> お前のimpact descriptorは**LITE**だ。どのシステムにも表示されない。だが俺の手元にはある。
>
> **3つのサークル**。第一：お前自身の成果。第二：誰かの仕事をより良くしたか？第三：チームの既存資産を活用したか？第一サークルには辛うじて何かある。第二と第三は空っぽだ。これはPrincipal SDEじゃない——L61のコストでL59の仕事をしている状態だ。
>
> 失敗後に振り返りなし。誰にも聞かず。何も変わっていない。それはグロースマインドセットじゃない——固定思考を3回繰り返しただけだ。マイクロソフトはすでにグロースマインドセットを報酬決定の入力に組み込んでいる。
>
> **GVSA**が送られた瞬間、2年間マイクロソフトのすべてのドアが閉まる——今のチームだけじゃない、全チームだ。知っている人間、組んだことのあるマネージャー、頼りにしていた社内紹介——全部使えなくなる。その記録はお前の名前に貼りついて、どこへ行っても追いかけてくる。

#### 🟩 マイクロソフト味·グロースマインドセット型（同じアプローチへの固執、新しい角度の拒否時）

> 同じアプローチ。3回目。まだ失敗している。
>
> マイクロソフトが**Exceptional Impact**に求めることで、お前が毎回スキップしているものがある：*挫折への個人的な説明責任を取り、フィードバックを求め、学びを行動に転化すること*。お前は何にも責任を取っていない。何も変えていない。何も学んでいない。これはSLITEじゃない——LITEだ。LITEになった時点で、お前の報酬の話し合いはもう終わっている。
>
> 次のConnectsはやり直しじゃない。この期間にお前がやったこと、やらなかったことの完全な記録だ。修正しようがしまいが、この記録は存在する。問題はそこに何が書かれているかだけだ。
>
> 直せ。検証しろ。3つのサークル全部見せろ。
